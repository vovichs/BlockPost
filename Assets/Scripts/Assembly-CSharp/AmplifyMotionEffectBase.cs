...
public class AmplifyMotionEffectBase : MonoBehaviour
{
	...
	{
		AmplifyMotionCamera[] array = Enumerable.ToArray(m_linkedCameras.Values);
	...
	private void LateUpdate()
	{
		...
		{
			float num = (Application.isPlaying ? Time.deltaTime : Time.fixedDeltaTime);
	...
	private void RenderReprojectionVectors(CommandBuffer commandBuffer, RenderTexture destination, float scale)
	{
		...
		commandBuffer.Blit(new RenderTargetIdentifier(tex), destination, m_reprojectionMaterial);
	}
	...
	private void OnPostRender()
	{
		...
		ResetObjectId();
		bool num = CameraMotionMult > float.Epsilon;
		bool clearColor = !num || m_starting;
		float y = ((DepthThreshold > float.Epsilon) ? (1f / DepthThreshold) : float.MaxValue);
