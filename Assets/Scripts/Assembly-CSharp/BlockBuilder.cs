...
public class BlockBuilder
{
	...
	private static Color _lc;

	private static Color _shadow = new Color(0.74f, 0.84f, 0.94f, 1f);

	private static Color _black = new Color(0.01f, 0.01f, 0.01f, 1f);
	...
	public static void Build(int flag, Vector3 localpos, Vector3 worldpos, Color color)
	{
		...
		for (_face = 0; _face < 6; _face++)
		{
			...
			if (!IsFaceVisible())
			{
				if (MapLight.data != null)
				{
					int num = MapLight.data[(int)_nearpos.x, (int)_nearpos.z];
					if ((int)_nearpos.y < num)
					{
						_lc = color * _shadow;
					}
					else
				...
				BuildFaceAO(flag, _face, localpos, _lc, _a[0], _a[1], _a[2], _a[3]);
			}
		}
	}
	...
	private static void BuildFace(int flag, int face, Vector3 localpos, Color color)
	{
		...
		MeshBuilder.AddVertices(vertices[face], localpos);
		MeshBuilder.AddNormals(normals[face]);
		MeshBuilder.AddTexCoords(Atlas.GetUV(flag, face));
		MeshBuilder.AddFaceColor(color);
	}
	...
	private static void BuildFaceAO(int flag, int face, Vector3 localpos, Color color, float a, float b, float c, float d)
	{
		if (a + c < b + d)
		{
			MeshBuilder.AddFaceIndices_flipped(0);
		}
		else
		{
			MeshBuilder.AddFaceIndices(0);
		...
		MeshBuilder.AddNormals(normals[face]);
		MeshBuilder.AddTexCoords(Atlas.GetUV(flag, face));
		MeshBuilder.AddFaceColor(color, a, b, c, d);
	}
	...
	private static void BuildFaceLine(int flag, int face, Vector3 localpos, Color color, int sizex, int sizez = 1)
	{
		...
		MeshBuilder.AddFaceIndices(0);
		MeshBuilder.AddVertices(vert, localpos);
		MeshBuilder.AddNormals(normals[face]);
		MeshBuilder.AddTexCoords(uv);
		MeshBuilder.AddFaceColor(color);
	}
	...
	public static void BuildTex(int flag, Vector3 pos, Color color)
	{
		for (int i = 0; i < 6; i++)
		{
			int num = i;
			...
			if (!IsFaceVisibleTex((int)vector2.x, (int)vector2.y, (int)vector2.z))
			{
				...
				{
				case 4:
					c = new Color(color.r + 0.15f, color.g + 0.15f, color.b + 0.15f);
					break;
				case 2:
				case 3:
					c = new Color(color.r + 0.1f, color.g + 0.1f, color.b + 0.1f);
					break;
				default:
					c = color;
					break;
				}
				BuildFace(flag, num, pos, c);
			}
		}
	}
	...
	public static void BuildFaceBlockTex(int face, Vector3 localpos, Color color, float sizex, float sizey, float sizez, float x0, float y0, float w, float h, bool rotate = false)
	{
		...
		localpos -= pivot;
		MeshBuilder.AddFaceIndices(matidx);
		MeshBuilder.AddVertices(vert, localpos);
		MeshBuilder.AddNormals(normals[face]);
		MeshBuilder.AddTexCoords(uv);
		MeshBuilder.AddFaceColor(color);
