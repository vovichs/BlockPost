...
namespace GooglePlayGames.BasicApi.Video
{
	public class VideoCapabilities
	{
		...
		{
			return string.Format("[VideoCapabilities: mIsCameraSupported={0}, mIsMicSupported={1}, mIsWriteStorageSupported={2}, mCaptureModesSupported={3}, mQualityLevelsSupported={4}]", mIsCameraSupported, mIsMicSupported, mIsWriteStorageSupported, string.Join(",", Enumerable.ToArray(Enumerable.Select(mCaptureModesSupported, (bool p) => p.ToString()))), string.Join(",", Enumerable.ToArray(Enumerable.Select(mQualityLevelsSupported, (bool p) => p.ToString()))));
		}
	}
}
